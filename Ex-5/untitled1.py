...
class cannon_shell_trajectory:
    def __init__(self, x=0, y=0, v=700,time_step=0.0001, g=9.8,theta=45, a=0.0065, 
alpha=2.5, B=0.00004, T=300):
        self.x=[x]
        self.y=[y]
        self.dt=time_step
        self.theta=theta
    ...
    def run(self):
        while(self.y[-1]>=0):
            self.v_x=self.v_x-self.B*self.v*self.v_x*math.pow(1-self.a*self.y[-1]/self.T,self.alpha)*self.dt
            self.x.append(self.x[-1]+self.v_x*self.dt)
            self.v_y=self.v_y-self.g*self.dt-self.B*self.v*self.v_y*math.pow(1-self.a*self.y[-1]/self.T,self.alpha)*self.dt
