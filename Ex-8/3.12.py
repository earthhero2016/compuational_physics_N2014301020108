...
import math
class chaos:
    def __init__(self,theta=0.2,omega=0,t=0,time_step=3*math.pi/1000,F_D=1.4,q=0.5,
l=9.8,g=9.8,OMEGA_D=2/3):
    ...
    def show2(self):       
        ...
        self.th1=self.th1[100:636]
        self.om1=self.om1[100:636]
        
        pl.subplot(221)
        pl.plot(self.th1,self.om1,'.')
        pl.title(r'                 $F_D=1.4$')
        pl.xlabel(r'$\theta$(rad)')
        pl.ylabel('$\omega$(rad/s)(0 phase)')   

        pl.subplot(222)
        pl.plot(self.th2[100:636],self.om2[100:636],'.')
        pl.xlabel(r'$\theta$(rad)')
        pl.ylabel('$\omega$(rad/s)($\pi$/4 phase)')
        pl.title(r'$\omega$ versus $\theta$')
    
        self.th3=self.th3[100:636]
        self.om3=self.om3[100:636]
        pl.subplot(223)
        pl.plot(self.th3,self.om3,'.')
        pl.xlabel(r'$\theta$(rad)')
        pl.ylabel('$\omega$(rad/s)($\pi$/2 phase)')
        

        pl.subplot(224)
        pl.plot(self.th4[100:636],self.om4[100:636],'.')
        pl.xlabel(r'$\theta$(rad)')
        pl.ylabel('$\omega$(rad/s)($3\pi$/4 phase)')
