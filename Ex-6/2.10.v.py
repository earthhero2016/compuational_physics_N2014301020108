# -*- coding: utf-8 -*-
"""
Created on Mon Oct 17 15:59:39 2016
...
class cannon_shell_trajectory:
    def __init__(self, x=0, y=0, v0=692,time_step=0.0001, g0=9.8,a=0.0065, 
alpha=2.5, B=0.00004, T=300,r=6371000,vw=-10,theta=30,x0=20000,y0=3000):
        self.x=[x]
        self.y=[y]
        self.dt=time_step
        self.v0=v0
      ...
      while(self.v<self.v0+16):        
           while(self.theta<60):
               ...
               while(self.y[-1]>self.y0):
                  ...
                  self.v_y=self.v_y-self.g*self.dt-self.B*math.sqrt(math.pow(self.v_x-self.vw,2)+math.pow(self.v_y,2))*self.v_y*math.pow(1-self.a*self.y[-1]/self.T,self.alpha)*self.dt
                  self.y.append(self.y[-1]+self.v_y*self.dt)   
                  if(self.x0-200<self.x[-1]<self.x0+200 and self.y[-1]<self.y0+10):
                      ...
                      while(self.theta<self.a1+4):
                          ...
                          while(self.y[-1]>self.y0):
                              ...
                              self.v_y=self.v_y-self.g*self.dt-self.B*math.sqrt(math.pow(self.v_x-self.vw,2)+math.pow(self.v_y,2))*self.v_y*math.pow(1-self.a*self.y[-1]/self.T,self.alpha)*self.dt
                              self.y.append(self.y[-1]+self.v_y*self.dt) 
                          if(self.x0-50<self.x[-1]<self.x0+50 and self.y[-1]<self.y0+10 ):
                              print(self.x[-1],self.y[-1],self.theta)
                              ...
                              while(self.theta<self.a2+1):
                                  ...
                                  while(self.y[-1]>self.y0):
                                      ...
                                      self.v_y=self.v_y-self.g*self.dt-self.B*math.sqrt(math.pow(self.v_x-self.vw,2)+math.pow(self.v_y,2))*self.v_y*math.pow(1-self.a*self.y[-1]/self.T,self.alpha)*self.dt
                                      self.y.append(self.y[-1]+self.v_y*self.dt) 
                                  if(self.x0-11<self.x[-1]<self.x0+11):
                                      ...
                                      pl.ylim(0,)
                                      pl.show()
                                      print(self.x[-1],self.y[-1],self.theta,self.v)
               self.x=[0]
               self.y=[0]
               self.theta+=2
               self.v_x=self.v*math.cos(math.radians(self.theta))
